...
clear.addEventListener("click", function(){clean(); window.cancelAnimationFrame(id); isMoving = false; isXtra = false;});
c.addEventListener("click", function(){draw()});
m.addEventListener("click", function(){if (isMoving) {window.cancelAnimationFrame(id); isMoving = false;} else {move();}});
xtra.addEventListener("click", function(){doXtra(isXtra)});
